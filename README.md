# ARG 초보자 튜토리얼 — 한국어판

일본어 Web 탐색형 ARG 연습 사이트의 한국어 팬 번역판.

- 원작: **まりc** — https://argtutorial.marimaric.workers.dev/
- 원작자 X: https://x.com/nstzg3Cq4s3rsAU
- 엔진: YACHO(SIM3)

비영리·무료. 원작 구조를 그대로 두고 텍스트만 한국어로 옮겼습니다.

## 상태

원작자 まりc 님께 번역·재게시 허락을 받고 진행했습니다.

| 항목 | 상태 |
|---|---|
| 원본 미러 | 완료 (23페이지 + 자산 49파일) |
| 번역 | **완료 (635/635)** |
| 한글 검색 인덱스 | 완료 (띄어쓰기 변형 별칭 포함) |
| 세로읽기 퍼즐 | 전·후반 모두 한국어로 재구성 |
| 진행 검증 | 키워드 18종 전부 통과 |
| 미해결 키워드 | maocool(ex1), soudesu(ex2) 2개 |

`maocool` 과 `soudesu` 는 원작에서도 본문으로는 추측할 수 없게 만든 보너스
페이지입니다. 키워드를 알게 되면 `tools/build_search.py` 의 `KO` 표에 한 줄
추가하고 다시 돌리면 됩니다.

## 종이접기 퍼즐 이미지

`yokuaru2` 의 사진 3장은 **원작자의 원본 사진을 그대로 쓰고, 빨간 원문 옆에
한글만 덧적었다.** 지우거나 다시 만들지 않았다.

그렇게 한 이유가 있다. 처음에는 빈 종이를 생성해 한글을 새로 배치했는데,
그러려면 종이가 몇 장인지, 어느 면이 뒤집혔는지를 추론해야 했고 두 번 틀렸다.
원본 위에 덧적으면 그 추론이 아예 필요 없다 — 있던 자리 옆에 쓰면 된다.

검은 글자는 전부 의미 없는 미끼라 일본어로 남겨 두었다. 빨강/검정 구분이
뚜렷해 헷갈리지 않고, 원작자의 종이가 온전히 보존된다.

  現実世界 → 현실세계   (검색 정답, 좌상→우하 대각선)
  しらべて → 조사해봐   (숨은 지시문, 우상→좌하 대각선)

`tools/handwrite_original.py` 가 칸별로 빨간 획의 무게중심을 찾아 그 옆에
붓 계열 글꼴로 한글을 얹는다. 곱하기로 합성해 종이 결과 접힘 그림자에 스민다.

## 원본과 다르게 한 곳

`unreasonable`, `yokuaru3` 의 **협력자 대사를 흰 글씨로 깔았다.** 원본에서는
그냥 보인다. 드래그해야 드러난다.

협력자는 막힌 사람에게 답을 흘려 주는 안전장치 역할인데, 그게 바로 눈에 들어와
다음 키워드와 숨은 링크 위치가 먼저 읽혀 버렸다. 흰 글씨로 깔면 안전장치는
그대로 남으면서 먼저 눈에 띄지는 않는다.

이 사이트가 `마리c` 페이지에서 직접 가르치는 기법이라 어울리기도 한다.
다만 원본에 없는 연출이므로 원작자에게 알려 두는 것이 좋다.

## 작업 방법

```bash
python tools/i18n.py extract              # 원문에서 문자열 카탈로그 생성
python tools/i18n.py dump contents/xxx    # 미번역 문자열을 인덱스와 함께 확인
python tools/i18n.py set tools/patch.json # {파일:{인덱스:한국어}} 반영
python tools/i18n.py apply                # 실제 파일에 주입
python tools/i18n.py status               # 진행률
python tools/build_search.py              # 한글 검색 인덱스 재생성
```

카탈로그(`tools/catalog.json`)가 원본 파일의 문자 위치를 들고 있어,
번역을 넣어도 태그·CSS·숨김 요소 구조는 건드리지 않습니다.

## 로컬 실행

정적 사이트지만 `fetch` 를 쓰므로 `file://` 로는 안 됩니다.

```bash
python -m http.server 4173 --directory .
```

## 현지화에서 조심할 것

- **세로읽기 퍼즐** — 9개 페이지에 한 글자씩 숨겨 `케로케로하트를검색` (9자)이
  되도록 배치합니다. 원문은 `ケロケロハート検索`. 글자 수가 어긋나면 퍼즐이 깨집니다.
- **검색 토큰 수** — 엔진은 공백으로 나눈 토큰을 2개까지만 봅니다.
  한글 정답은 붙여 쓰고, 띄어쓰기 변형은 별칭 파일로 받습니다.
- **숨김 요소** — `display:none` 등으로 감춘 단서가 페이지마다 있습니다.
  내용만 번역하고 구조는 그대로 두세요.
- **`noindex` 유지** — 스포일러 방지.

자세한 설계는 [docs/superpowers/specs/2026-09-09-arg-tutorial-kr-design.md](docs/superpowers/specs/2026-09-09-arg-tutorial-kr-design.md).
