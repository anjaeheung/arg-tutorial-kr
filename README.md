...
`yokuaru2` 의 사진 3장은 **원작자의 원본 사진을 그대로 쓰고, 빨간 원문 옆에
한글만 덧적었다.** 지우거나 다시 만들지 않았다.

그렇게 한 이유가 있다. 처음에는 빈 종이를 생성해 한글을 새로 배치했는데,
그러려면 종이가 몇 장인지, 어느 면이 뒤집혔는지를 추론해야 했고 두 번 틀렸다.
원본 위에 덧적으면 그 추론이 아예 필요 없다 — 있던 자리 옆에 쓰면 된다.

검은 글자는 전부 의미 없는 미끼라 일본어로 남겨 두었다. 빨강/검정 구분이
뚜렷해 헷갈리지 않고, 원작자의 종이가 온전히 보존된다.

  現実世界 → 현실세계   (검색 정답, 좌상→우하 대각선)
  しらべて → 조사해봐   (숨은 지시문, 우상→좌하 대각선)

`tools/handwrite_original.py` 가 칸별로 빨간 획의 무게중심을 찾아 그 옆에
붓 계열 글꼴로 한글을 얹는다. 곱하기로 합성해 종이 결과 접힘 그림자에 스민다.

## 원본에 없는 추가

`yokuaru2` 에 **직접 접어보기**(`/fold/`) 링크를 붙였다. 원본에는 없다.
사진 3장만으로 퍼즐은 그대로 완결되고, 이건 선택지다.

종이접기 3D 툴(anjaeheung/paper-fold)을 복사해 `assets/1-front.jpg`(흰 종이),
`2-front.jpg`(노란 종이)를 심어 두었다. 이 퍼즐은 종이 두 장이라 한 장의
앞뒤 모델인 툴로는 겹치기를 재현하지 못한다. 접는 동작이 일어나는 건 흰
종이뿐이므로 그것만 접어 보고, 노란 종이는 따로 넘겨 본다.
